...
namespace MMILauncher.Core
{
    ...
    public class MMIRegisterServiceImplementation : MMIRegisterService.Iface
    {
        ...
        public virtual Dictionary<MMUDescription, List<MIPAddress>> GetAvailableMMUs(string sessionID)
        {
            ...
            Logger.Log(Log_level.L_INFO, $"Request for available MMUs processed. {dict.Count} available MMUs reported");
            return dict;
        }
        ...
        {
            List<MAdapterDescription> l = RuntimeData.AdapterInstances.Values.Select(s => s.Description).ToList();
            //Logger.Log(Log_level.L_INFO, $"Request for Registered Adapters. Reporting {l.Count} Adapters");
            return l;
        }
        ...
        {
            var l = RuntimeData.ServiceInstances.Values.Select(s => s.Description).ToList();
            //Logger.Log(Log_level.L_INFO, $"Request for Registered Services. Reporting {l.Count} Services");
            return l;
        }
        ...
        public virtual MBoolResponse RegisterAdapter(MAdapterDescription adapterDescription)
        {
            ...
            {
                Logger.Log(Log_level.L_INFO, $"Adapter: {adapterDescription.Name} already available -> nothing to do.");
            ...
            {
                Logger.Log(Log_level.L_ERROR, $"Cannot add adapter description {adapterDescription.Name}");
            }
            
            Logger.Log(Log_level.L_INFO, $"Adapter {adapterDescription.Name} registered");
            //Fire event
        ...
        public MBoolResponse RegisterService(MServiceDescription serviceDescription)
        {
            ...
            {
                Logger.Log(Log_level.L_INFO, $"Service: {serviceDescription.Name} already available -> nothing to do.");
                return new MBoolResponse(true);
            }
            ...
            {
                Logger.Log(Log_level.L_ERROR, $"Cannot add service description {serviceDescription.Name}");
            }
            Logger.Log(Log_level.L_INFO, $"Registered service {serviceDescription.Name}");
        ...
        public MBoolResponse UnregisterAdapter(MAdapterDescription adapterDescription)
        {
            ...
            if (RuntimeData.AdapterInstances.ContainsKey(adapterDescription.ID))
            {
                ...
                Logger.Log(Log_level.L_INFO, $"Adapter {adapterDescription.Name} unregistered");
            ...
            Logger.Log(Log_level.L_ERROR, "No matching adapter to unregister found");
        ...
        public MBoolResponse UnregisterService(MServiceDescription serviceDescription)
        {
            ...
            if (RuntimeData.ServiceInstances.ContainsKey(serviceDescription.ID))
            {
                ...
                Logger.Log(Log_level.L_INFO, $"Service {serviceDescription.Name} unregistered");
            ...
            Logger.Log(Log_level.L_ERROR, $"No matching service for {serviceDescription.Name} found!");
            return new MBoolResponse(false)
            {
