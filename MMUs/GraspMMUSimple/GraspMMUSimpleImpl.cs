...
namespace GraspMMUSimple
{
    ...
    public class GraspMMUSimpleImpl : MMUBase
    {
        ...
        public override MBoolResponse Initialize(MAvatarDescription avatarDescription, Dictionary<string, string> properties)
        {
            ...
            MMICSharp.Logger.Log( MMICSharp.Log_level.L_INFO, path);
            //Load the closed hand posture for left and right hand
            if (!System.IO.File.Exists(path +"/leftHandClosed.json"))
            {
                MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Left hand cannot be loaded. Please ensure that leftHandClosed.json is in the same folder as the MMU");
            ...
            {
                MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Right hand cannot be loaded. Please ensure that righHandClosed.json is in the same folder as the MMU");
                return new MBoolResponse(false);
            }
        ...
        public override MBoolResponse AssignInstruction(MInstruction instruction, MSimulationState simulationState)
        {
            ...
            if (!instruction.Properties.ContainsKey("HandPose"))
            {
                ...
                MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Mandatory parameter HandPose is not defined (GraspMMUSimple)");
            ...
            {
                //Return false if the hand parameter is not set
                MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Mandatory parameter Hand is not defined (GraspMMUSimple)");
            ...
            try
            {
                ...
                switch (instruction.Properties["Hand"])
                {
                    ...
                    default:
                        //Return false if the hand parameter is not set
                        MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Unknown Hand defined (either Left or Right) (GraspMMUSimple)");
                        return new MBoolResponse(false);
                }
            }
            catch (Exception e)
            {
                MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Exception occured, aborting MMU (GraspMMUSimple). " + e.Message + " " + e.StackTrace);
                return new MBoolResponse(false);
            }
        ...
        private bool SetupHand(MJointType jointType, MInstruction instruction, float duration, MSimulationState simulationState)
        {
            ...
            if (instruction.Properties.ContainsKey("HandPose"))
            {
                ...
                {
                    MMICSharp.Logger.Log(MMICSharp.Log_level.L_ERROR, "Problem getting HandPose, please ensure that a PostureConstraint is set for the HandPose with the defined constraint ID " + constraintID);
                    return false;
                }
            }
            ...
            for (float w = 0; w < 1; w += 1f/(float)iterations)
            {
                ...
                foreach (Finger finger in hand.Fingers)
                {
                    ...
                    //MMICSharp.Logger.Log(MMICSharp.Log_level.L_DEBUG, finger.JointTypes[0] + " " + distance);

                    if (distance < finger.MinDistance)
                    {
                        MMICSharp.Logger.Log(MMICSharp.Log_level.L_INFO, finger.JointTypes[0] + " " + distance);
